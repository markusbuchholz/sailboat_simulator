...
# Program accepts constant flow of waypoints from terminal and perform dynamic position
    ...
    for idx, (x, y) in enumerate(positions):
        ...
        while time.time() - start_time < timeout:
            ...
            if msg:
                ...
                if abs(current_lat - lat) < 0.00005 and abs(current_lon - lon) < 0.00005:  # increased tolerance for GPS !!!! MIXME if needed
        ...
        # Hold position after reaching the target
        set_mode('LOITER')
        time.sleep(2)
    ...
    while True:
        ...
        if user_input.strip():
            try:
                ...
                print("Invalid input format. Please enter positions as 'X,Y'.")
        else:
