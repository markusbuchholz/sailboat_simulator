...
class SimpleYawController(Node):
    ...
    def move_to_waypoint(self):
        ...
        #self.get_logger().info(f"Angular PID thrust 1: {angular_velocity}")
        angular_effort = max(min(angular_velocity, 20.0), -20.0)
        #self.get_logger().info(f"Angular PID thrust 2: {angular_effort}")
        
        #if -7.0 < angular_effort < 7.0:
        #    angular_effort = 0.0       
        
        if abs(heading_error) <= 4.5 : #(self.heading_tolerance + 5.0):
            angular_effort = 0.0
    ...
    def publish_thrust_YZ(self, thrust_fwd, thrust_rot):
        #self.get_logger().info(f"Linear 2: {thrust_fwd}, Angular 2: {thrust_rot}")
        ...
        # if abs(thrust_rot) > rot_threshold:
        #     self.get_logger().info("_____________Moving Forward_ROT______________")
        #     # If rotational thrust exceeds threshold, apply reduction to forward movement
        #     reduction_factor = 0.0001  # Adjust this factor based on desired reduction
        #     cmd_vel_msg.linear.z = 0.0 #thrust_fwd_effort * 0.02 * reduction_factor
        #     cmd_vel_msg.linear.y = -thrust_rot * 2.0 #* self.wave_control
        # else:
        #     self.get_logger().info("_____________Moving Forward_LIN______________")
            
        #     # normal operation without rotational thrust adjustment
        #     cmd_vel_msg.linear.z = thrust_fwd_effort * 0.02
        #     cmd_vel_msg.linear.y = 0.0 #-thrust_rot * 3.0
