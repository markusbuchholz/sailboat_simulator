...
class MavlinkControlNode(Node):
    def __init__(self):
        ...
        self.setup_message_request_intervals()
        
    
    
        
    def setup_message_request_intervals(self):
        self.request_message_interval(mavutil.mavlink.MAVLINK_MSG_ID_LOCAL_POSITION_NED, 1000000)  # 1 Hz
        self.request_message_interval(mavutil.mavlink.MAVLINK_MSG_ID_ATTITUDE, 1000000)  # 1 Hz

    def request_message_interval(self, msg_id, interval_us):
        """Request periodic message updates at the specified interval in microseconds."""
        self.get_logger().info(f"Requesting {msg_id} at interval {interval_us} microseconds")
    ...
    def has_moved_significantly(self, current_position, threshold=0.5):
        if self.last_published_position is None:
            return True
        distance = math.sqrt(
            (current_position['x'] - self.last_published_position['x'])**2 +
            (current_position['y'] - self.last_published_position['y'])**2 +
            (current_position['z'] - self.last_published_position['z'])**2
        )
    ...
    def fetch_current_state_local(self):
        """Fetches current state, attempting multiple times if necessary."""
        max_attempts = 5
        position = None
        orientation = None

        for attempt in range(max_attempts):
            self.get_logger().info(f"Attempting to fetch position data... Attempt {attempt + 1}/{max_attempts}")
            ...
            if position_message:
                ...
                self.get_logger().warn(f"No valid position received on attempt {attempt + 1}")
                sleep(1)

        if not position:
            self.get_logger().error("Failed to fetch valid position after several attempts")
            raise TimeoutError("Failed to get a valid position")

        for attempt in range(max_attempts):
            self.get_logger().info(f"Attempting to fetch attitude data... Attempt {attempt + 1}/{max_attempts}")
            attitude_message = self.vehicle.recv_match(type='ATTITUDE', blocking=True, timeout=5)
            if attitude_message:
                orientation = {'yaw': attitude_message.yaw}
                break
            else:
                self.get_logger().warn(f"No valid attitude received on attempt {attempt + 1}")
                sleep(1)

        if not orientation:
            self.get_logger().error("Failed to fetch valid attitude after several attempts")
            raise TimeoutError("Failed to get a valid attitude")
