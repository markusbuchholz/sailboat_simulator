...
class CoordinateApp:
    ...
    def plan_path(self):
        ...
        if not self.polygon:
            messagebox.showerror("Error", "Please define a polygon area for path planning.")
            return
    ...
    def save_path(self, complete_path, filename):
        with open(filename, "w") as file:
            num_points = len(complete_path)
            step = max(1, num_points // self.max_points)
            selected_points = [complete_path[i] for i in range(0, num_points, step)]
            selected_points = selected_points[:self.max_points-1]  # Ensure we do not exceed max_points
            selected_points.append(complete_path[-1])  # Always include the goal point
            for point in selected_points:
                x, y = point
                scaled_x, scaled_y = x * self.scale_factor, y * self.scale_factor
                file.write(f"{scaled_x}, {scaled_y}\n")
    ...
    def save_simple_path(self):
        ...
        with open("mission_path.txt", "w") as file:
            file.write(f"{self.start[0] * self.scale_factor}, {self.start[1] * self.scale_factor}\n")
            for waypoint in self.waypoints:
                file.write(f"{waypoint[0] * self.scale_factor}, {waypoint[1] * self.scale_factor}\n")
            file.write(f"{self.goal[0] * self.scale_factor}, {self.goal[1] * self.scale_factor}\n")
    ...
    def rrt_star(self, start, goal, without_environmental_cost=False):
        ...
        for i in range(10000):
            random_point = self.random_point_in_polygon()
            if random_point is None:
                continue
            nearest_node = self.get_nearest_node(nodes, random_point)
            new_node = self.steer(nearest_node, random_point)
            if not self.is_colliding(nearest_node, new_node) and self.is_path_in_polygon(nearest_node, new_node):
    ...
    def random_point_in_polygon(self):
        min_x, min_y = np.min(self.polygon_points, axis=0)
        max_x, max_y = np.max(self.polygon_points, axis=0)
        attempts = 0
        while attempts < 10000:
            point = (random.uniform(min_x, max_x), random.uniform(min_y, max_y))
            if self.polygon.contains(Point(point)):
                return self.canvas_to_grid(*point)
            attempts += 1
        return None
    ...
    def is_path_in_polygon(self, from_node, to_node):
        x1, y1 = self.grid_to_canvas(*from_node)
        x2, y2 = self.grid_to_canvas(*to_node)
        if x1 == x2 and y1 == y2:  # Prevent division by zero
            return True
        if not self.polygon.contains(Point((x1, y1))) or not self.polygon.contains(Point((x2, y2))):
            return False
        steps = max(int(max(abs(x2 - x1), abs(y2 - y1)) / self.grid_size), 1)  # Ensure steps is at least 1
    ...
    def a_star(self, start, goal, without_environmental_cost=False):
        ...
        while open_set:
            ...
            for neighbor in self.get_neighbors(current):
                if not self.is_point_in_polygon(neighbor):
                    continue
                if not self.is_path_in_polygon(current, neighbor):
                    continue
    ...
    def is_point_in_polygon(self, point):
        canvas_point = self.grid_to_canvas(*point)
        if self.polygon is None:
            return False
        return self.polygon.contains(Point(canvas_point))
    ...
    def clear_paths_and_obstacles(self):
        self.canvas.delete("all")
        self.start = None
        self.goal = None
        self.waypoints = []
    ...
    def run_sitl(self):
        try:
            with open("mission_path.txt", "r") as file:
                lines = file.readlines()
                positions = ";".join([line.strip() for line in lines])
            command = ["python3", "args_mission_planner_gps.py", "--positions", positions]
            subprocess.run(command, check=True)
            messagebox.showinfo("Success!", "SITL executed successfully.")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to execute SITL: {e}")
    ...
    def replan_polygon(self):
        ...
        self.canvas.delete("all")
        self.draw_polygon(new_polygon_points, "green")
    ...
    def avoid_obstacle(self, points, obstacle):
        ...
        if not polygon.intersects(obstacle_buffer):
            return points

        new_polygon = polygon.difference(obstacle_buffer)
        if isinstance(new_polygon, MultiPolygon):
            largest_poly = max(new_polygon, key=lambda p: p.area)
            new_polygon = largest_poly

        return list(new_polygon.exterior.coords)[:-1]
