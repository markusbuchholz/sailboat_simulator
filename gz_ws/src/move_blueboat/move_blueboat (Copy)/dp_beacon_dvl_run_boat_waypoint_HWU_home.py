...
class WayASVController(Node):
    def __init__(self):
       ...
       # self.odometry_subscription = self.create_subscription(Odometry, '/model/blueboat/odometry', self.odometry_callback, 10, callback_group=self.callback_group)
        ...
        self.subscription = self.create_subscription(Imu, '/blueboat/imu/data', self.odometry_callback, custom_qos, callback_group=self.callback_group)
        
        self.position_subscription = self.create_subscription(HedgePosition,'/hedgehog_pos',self.position_callback, subscriber_qos_profile,callback_group=self.callback_group)
    ...
    def odometry_callback(self, msg):
        ...
        #roll, _, _ = self.euler_from_quaternion(orientation_q)
    ...
    def publish_twist(self, thrust_fwd, thrust_rot):
        self.get_logger().info(f"Linear: {thrust_fwd}, Angular: {thrust_rot}")
        cmd_vel_msg = Twist()
        cmd_vel_msg.linear = Vector3()
        cmd_vel_msg.angular = Vector3()
        ...
        cmd_vel_msg.linear.z = 0.0 #thrust_fwd_effort* 0.02
        
        
        # implement binary swtich left/right
    ...
    def gazebo_publish_twist(self, linear_x, angular_z):
    ...
    def stop_asv(self):
        # cmd_vel_msg.linear.z = -thrust_fwd_effort* 0.03
