...
class WaypointFollower(Node):
    ...
    def waypoint_callback(self, msg):
        ...
        if self.target_waypoints:
            self.move_towards(self.target_waypoints[self.current_waypoint_index])
    ...
    def timer_callback(self):
        ...
        if distance_to_waypoint < 2.0:  # threshold distance to consider waypoint reached
            self.current_waypoint_index += 1
            if self.current_waypoint_index >= len(self.target_waypoints):
                self.get_logger().info("All waypoints reached.")
                self.target_waypoints = []  # Clear waypoints after reaching the last one
                return
