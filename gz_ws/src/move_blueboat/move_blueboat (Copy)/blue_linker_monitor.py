...
class PositionMonitor(Node):
    def __init__(self):
        ...
        # QoS Profiles
        self.custom_qos_odom = QoSProfile(
            reliability=QoSReliabilityPolicy.RELIABLE,
            durability=QoSDurabilityPolicy.VOLATILE,  # Change from TRANSIENT_LOCAL to VOLATILE
            depth=10
        )
        ...
        self.error_threshold = 0.5 ########0.7  #######################################

    def odometry_callback(self, msg):
        theta_deg = 0.0
        theta_radians = np.radians(theta_deg)
        #print (theta_radians)
        ...
        #current_position = (
        #    6.0, #(9.5 - abs(msg.pose.pose.position.x + self.x_offset)) * 1.6,
        #    6.5 - abs(msg.pose.pose.position.x) #abs(msg.pose.pose.position.x * cos_theta - msg.pose.pose.position.y * sin_theta))
        ...
        self.get_logger().info(
            f"Adjusted position: {current_position}, Adjusted orientation angle: {math.degrees(current_orientation):.2f} degrees")
