...
class SimpleYawController(Node):
    def __init__(self):
       ...
       # self.subscription = self.create_subscription(Imu,'/blueboat/imu/data',  self.listener_callback, custom_qos, callback_group=self.callback_group)
        ...
        self.position_tolerance = 2.0  # 1 meter tolerance
    ...
    def move_to_waypoint(self):
        ...
        if -7.0 < angular_effort < 7.0:
            angular_effort = 0.0       
        
        if abs(heading_error) <= 10.0 : #(self.heading_tolerance + 5.0):
            angular_effort = 0.0
            
        self.get_logger().info(f"Linear: {linear_velocity}, Angular: {angular_effort}")
        #simiar to joystick
    ...
    def dynamic_position(self):
        ...
        if  abs(heading_error) > self.dp_error_yaw:  # Adjust thresholds as needed
            ...
            angular_velocity = max(min(angular_velocity, 20.0), -20.0)
    ...
    def publish_thrust_YZ(self, thrust_fwd, thrust_rot):
        ...
        thrust_fwd_effort = max(min(thrust_fwd, 25.0), -25.0)
        
        if abs(thrust_rot) > rot_threshold:
            # If rotational thrust exceeds threshold, apply reduction to forward movement
            reduction_factor = 0.5  # Adjust this factor based on desired reduction
            cmd_vel_msg.linear.z = thrust_fwd_effort * 0.02 * reduction_factor
            cmd_vel_msg.linear.y = -thrust_rot * 3.0
        else:
            # Normal operation without rotational thrust adjustment
    ...
    def publish_thrust_ffd(self, thrust_fwd, thrust_rot):
        cmd_vel_msg = Twist()
        cmd_vel_msg.linear = Vector3()
        cmd_vel_msg.angular = Vector3()
    ...
    def stop_asv(self):
        ...
        #time.sleep(1)
        self.publish_twist_back(60.0, 60.0)
    ...
    def publish_twist(self, linear_x, angular_z):
        ...
        limit = 15.0
        thrust_fwd_1 = max(min(port_0_100, limit), -limit)
        thrust_fwd_2 = max(min(stbd_0_100, limit), -limit)
        #self.get_logger().info(f"Publishing thrust: Port={port_0_100}, Starboard={stbd_0_100}")
        self.get_logger().info(f"Publishing thrust: Port={thrust_fwd_1}, Starboard={thrust_fwd_2}")

        port_0_100_msg = Float32()
        stbd_0_100_msg = Float32()
        port_0_100_msg.data = thrust_fwd_1 #18.0 #float(port_0_100)
        stbd_0_100_msg.data = thrust_fwd_2 #18.0 #float(stbd_0_100)
