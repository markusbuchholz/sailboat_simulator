...
class SimpleYawController(Node):
    def __init__(self):
        ...
        # New variables for time-based constraints
        self.time_per_meter = 1.0  # 1 second per meter
        self.start_time = None
        self.allowed_duration = None
    ...
    def waypoint_callback(self, msg):
        ...
        if self.current_position:
            ...
            # Calculate the allowed duration based on the distance and time per meter
            self.allowed_duration = distance_to_target * self.time_per_meter
    ...
    def move_to_waypoint(self):
        if self.target_position:
            ...
            # Check if the elapsed time has exceeded the allowed duration
            elapsed_time = time.time() - self.start_time
            if elapsed_time > self.allowed_duration:
                self.get_logger().info("Time limit exceeded. Stopping thrusters.")
                self.stop_asv()
                return
