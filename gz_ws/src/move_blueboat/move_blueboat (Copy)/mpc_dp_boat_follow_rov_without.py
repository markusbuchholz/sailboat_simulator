...
class DynamicModel:
    def __init__(self, initial_upsilon=None, initial_eta=None):
        ...
        self.A = 4.0  # Wave amplitude
        self.Vw = 4.0  # Wind speed (m/s)
        self.Vc = 1.0  # Speed of the ocean current (m/s)
    ...
    def function_1(self, upsilon, time):
        ...
        #with distrubances
        Fx_dist = np.clip(T[0] - (Xu + Xuu * np.abs(upsilon[0])) + F_wave_x + F_wind_x + current_velocity_x, -1e6, 1e6)
        Fy_dist = np.clip(T[1] - (Yv * upsilon[1] + Yr * upsilon[2]) + F_wave_y + F_wind_y + current_velocity_y, -1e6, 1e6)

        Fx = np.clip(T[0] - (Xu + Xuu * np.abs(upsilon[0])), -1e6, 1e6)
        Fy = np.clip(T[1] - (Yv * upsilon[1] + Yr * upsilon[2]), -1e6, 1e6)
        
        print("fx:", Fx,  ", fx_DIST:",  Fx_dist,  ", fy:", Fy,  ", fy_DIST:", Fy_dist)
...
class WayASVController(Node):
    def __init__(self):
        ...
        self.position_monitoring = False  # New: To monitor the position
        self.registered_position = None  # New: To store the registered position
        self.position_threshold = 2.0  # New: Position error threshold
    ...
    def odometry_callback(self, msg):
        ...
        # Check position error if monitoring is active
        if self.position_monitoring:
    ...
    def navigate_to_waypoint(self):
        ...
        if self.state == 'rotate_to_final_heading':
            self.stop_asv()
            self.start_position_monitoring()  # New: Start monitoring position
            self.state = 'idle'
    ...
    def start_position_monitoring(self):
        """Start monitoring the current position to detect significant drift."""
        self.registered_position = self.dp_position ##################self.current_position
        self.position_monitoring = True
        self.get_logger().info("Position monitoring activated.")

    def stop_position_monitoring(self):
        """Stop monitoring the position."""
        self.position_monitoring = False
        self.get_logger().info("Position monitoring deactivated.")

    def check_position_error(self):
        """Check if the robot has drifted from its registered position."""
        if not self.registered_position:
            return

        distance_error = self.calculate_distance(self.current_position, self.registered_position)
        self.get_logger().info(f"DP error:::  {distance_error}")
        if distance_error > self.position_threshold:
            self.get_logger().warning(f"Position drift detected: {distance_error:.2f} meters. Re-registering current position as waypoint.")
            self.stop_position_monitoring()
            self.re_register_position_as_waypoint()

    def re_register_position_as_waypoint(self):
        """Publish the current position as a new waypoint."""
        waypoint_msg = Float64MultiArray()
        #waypoint_msg.data = [self.current_position[0], self.current_position[1]]
        waypoint_msg.data = [self.dp_position[0], self.dp_position[1]]
        self.waypoint_callback(waypoint_msg)  # Call the waypoint callback directly with the new waypoint
        self.get_logger().info("Current position re-registered as a waypoint.")
