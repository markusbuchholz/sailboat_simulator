...
class SimpleYawController(Node):
    def __init__(self):
        ...
        self.srv = self.create_service(SetBool, '/control_boat_movement', self.control_rov_callback)
        ...
        self.check_boat_publisher = self.create_publisher(Bool, '/check_boat', qos_profile=custom_qos)
        ...
        self.position_tolerance = 0.5  # 0.75 meter tolerance
        
        #Core control values
        self.control_heading_error = 1.0  
        self.velo_effort_loop = 0.023#0.025 ##################### FOWRARD
        self.rotating_speed = 4.1
    ...
    def control_rov_callback(self, request, response):
        # Manual control via client request
        if request.data != self.current_state:
            self.current_state = request.data
            self.get_logger().info(f'Boat set ROV movement to: {"STOP" if request.data else "MOVE"}')
        response.success = True
        response.message = 'ROV movement status updated successfully'
        return response
    ...
    def publish_cmd_vel(self, port_thrust, stbd_thrust=None):
        ...
        cmd_vel_msg.linear.y = -port_thrust * self.rotating_speed
    ...
    def move_to_waypoint(self):
        if self.target_position:
            ...
            if distance_to_target < self.position_tolerance:  # Threshold distance to stop
                ...
                if self.current_state:  # If ROV is currently stopped
                    self.current_state = False  # Allow ROV to move
                    self.get_logger().info('Boat reached the goal. Setting ROV to MOVE.')
