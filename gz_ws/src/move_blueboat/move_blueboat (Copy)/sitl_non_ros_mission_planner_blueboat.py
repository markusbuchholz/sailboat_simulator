...
class CoordinateApp:
    def __init__(self, root):
        ...
        self.sitl_button.grid(row=19, column=0, sticky='nsew')
    ...
    def is_path_in_polygon(self, from_node, to_node):
        ...
        if x1 == x2 and y1 == y2:  # Prevent division by zero
            return True
        if not self.polygon.contains_point((x1, y1)) or not self.polygon.contains_point((x2, y2)):
            return False
        steps = max(int(max(abs(x2 - x1), abs(y2 - y1)) / self.grid_size), 1)  # Ensure steps is at least 1
        for i in range(steps + 1):
            x = x1 + (x2 - x1) * i / steps
            y = y1 + (y2 - y1) * i / steps
            if not self.polygon.contains_point((x, y)):
                return False
        return True
    ...
    def is_point_in_polygon(self, point):
        ...
        if self.polygon is None:
            return False
        return self.polygon.contains_point(canvas_point)
