...
class SimpleYawController(Node):
    def __init__(self):
        ...
        self.control_heading_error = 4.0 #########################################################4.5
        self.dp_error_yaw = 5.0
        self.dp_error_distance = 300.0
        ...
        self.num_virtual_waypoints = 5  # Number of virtual waypoints to create
        
        self.previous_time = time.time()  # Initialize previous_time
    ...
    def waypoint_callback(self, msg):
        ...
        if self.current_position:
            ...
            target_yaw_world = self.normalize_angle(target_yaw_world + self.marvelmind_yaw)  # Adjust for 180-degree rotation
    ...
    def process_next_waypoint(self):
        if self.virtual_waypoints:
            next_wp = self.virtual_waypoints[0]
    ...
    def adjust_orientation(self):
        ...
        if abs(heading_error) <= self.heading_tolerance:
            ...
            self.rotating = False
            if self.virtual_waypoints:
                self.virtual_waypoints.pop(0)  # Remove the current virtual waypoint
    ...
    def move_to_waypoint(self):
        if self.virtual_waypoints:
            next_wp = self.virtual_waypoints[0]
            dx = next_wp[0] - self.current_position[0]
            dy = next_wp[1] - self.current_position[1]
            distance_to_target = math.sqrt(dx ** 2 + dy ** 2)
            self.get_logger().info(f"Distance to WP: {distance_to_target:.2f} meters")
        else:
            dx = self.target_position[0] - self.current_position[0]
            dy = self.target_position[1] - self.current_position[1]
            distance_to_target = math.sqrt(dx ** 2 + dy ** 2)
    ...
    def publish_thrust_YZ(self, thrust_fwd, thrust_rot):
        ...
        if abs(thrust_rot) > rot_threshold:
            ...
            cmd_vel_msg.linear.y = -thrust_rot * 1.5 ################################################## 
   ...
   ###############################################        
    def run_dynamic_position(self):
        ...
        self.get_logger().info(f"Current yaw: {self.current_roll:.2f} degrees, Reference yaw: {self.dp_reference_yaw:.2f} degrees")
        ...
        # Compensate position if distance error is large
        if distance_error > self.dp_error_distance:
            self.move_to_dp_position()

    def move_to_dp_position(self):
        dx = self.dp_reference_position[0] - self.current_position[0]
        dy = self.dp_reference_position[1] - self.current_position[1]
        distance_to_target = math.sqrt(dx ** 2 + dy ** 2)
        self.get_logger().info(f"Distance to DP reference position: {distance_to_target:.2f} meters")

        if distance_to_target < self.position_tolerance:  # Threshold distance to stop
            self.get_logger().info("DP reference position reached. Stopping thrusters.")
            self.rotating = False
            self.moving_to_waypoint = False  # Stop moving to the waypoint
            self.dynamic_position = True
            self.linear_integral = 0.0  # Reset integral term
        ...
        linear_velocity = self.calculate_pid(self.linear_kP, self.linear_kI, self.linear_kD, distance_to_target, self.linear_prev_error, self.linear_integral, self.timer_period)
        angular_velocity = self.calculate_pid(self.angular_kPm, self.angular_kIm, self.angular_kDm, heading_error, self.prev_error, self.integral, self.timer_period)
        angular_effort = max(min(angular_velocity, 20.0), -20.0)
